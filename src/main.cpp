...
int main() {
	...
	int time_steps_before_lock_required = 100; // number of time steps before accuracy is printed and checked as this may be time particle filter error takes to converge
	double max_runtime = 45; // Max allowed  runtime to pass in secs 
	double max_translation_error = 1; // Max allowed  translation error to pass in meters
	double max_yaw_error = 0.05; // Max allowed yaw error in radians
	...
	//Parameters setup
	double delta_t = 0.1; // Time between two consecutive measurements in seconds
	double sensor_range = 50; // Range of the sensor in meters
	...
	// Noise being generated using default_random_engine gen function random generator and normal_distribution<double>
		...
		if (i >= time_steps_before_lock_required) {
			if (cum_mean_error[0] > max_translation_error || cum_mean_error[1] > max_translation_error || cum_mean_error[2] > max_yaw_error) {
				if (cum_mean_error[0] > max_translation_error) {
					cout << "X error, " << cum_mean_error[0] << " is more than the maximum allowable error, " << max_translation_error << endl;
				}
				else if (cum_mean_error[1] > max_translation_error) {
					cout << "Y error, " << cum_mean_error[1] << " is more than the maximum allowable error, " << max_translation_error << endl;
				}
				else {
					cout << "Yaw error, " << cum_mean_error[2] << " is more than the maximum allowable error, " << max_yaw_error << endl;
				}
				return -1;
			}
		}
	...
	// Runtime for the filter being printed out.
	int stop = clock();
	...
	if (runtime < max_runtime && pf.initialized()) {
		...
		cout << "Starter code. filter have not been intialised." << endl;
	}
	else {
		cout << "Your runtime " << runtime << " is more than the maximum allowable runtime, " << max_runtime << endl;
		return -1;
	}
